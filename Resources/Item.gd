...
@export var icon: Texture ## An Icon for both the Overworld item and possibly the Inventory one.
@export var bonus: bool = false ## Probably not going to be used. Made it anyway in case I change my mind.
@export var durability: float: ## Setting this Value to -1 makes it Unbreakable.
	set(new_val):
		if durability == -1:
			pass
		else:

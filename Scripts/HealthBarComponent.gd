...
func _ready() -> void: # INFO: Death Print is here.
...
func health_changed(current_health: float) -> void:
	...
	tween.tween_property(health_bar, "size:x", max_health_bar_size * health_percentage, 0.05).set_ease(Tween.EASE_OUT).set_trans(Tween.TRANS_CIRC)
